...
function getVersion(): string {
  const envVer =
    process.env.KRONOS_CLI_VERSION ||
    process.env.POKT_CLI_VERSION ||
...
/** Arte ASCII estilo bloco (ANSI Shadow / mesmo espírito do antigo POKT CLI). */
...
function bannerLines(ver: string): string[] {
  return [...BANNER_ART_LINES, `  Version v${ver}`];
}
...
export const ui = {
  ...
  shortcutsLine: (
    left: string = "shift+tab para aceitar edições",
    right: string = "? para atalhos",
    center?: string
  ) => {
    ...
    if (c)
      return (
        l +
        " ".repeat(Math.max(0, 40 - left.length - cl)) +
        c +
        " ".repeat(Math.max(0, 20 - right.length)) +
        r
      );
  ...
  statusBar: (opts: { cwd?: string; branch?: string; sandbox?: string; model?: string }) => {
    ...
    const sandbox =
      opts.sandbox ?? chalk.red("no sandbox") + chalk.gray(" (see /docs)");
